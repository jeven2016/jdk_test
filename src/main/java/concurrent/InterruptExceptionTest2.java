...
public class InterruptExceptionTest2 {
  public static void main(String[] args) {
    ...
    thread.interrupt();
    System.out.println("the last state :" + thread.isInterrupted());

    try {
      //当线程中断后状态被清理后，再此调用将显示为false
      TimeUnit.SECONDS.sleep(2);
      System.out.println("the last state2 :" + thread.isInterrupted());
      System.out.println("the last state2 :" + thread.isInterrupted());
      System.out.println("it is alive : :" + thread.isAlive());
    } catch (InterruptedException e) {
      e.printStackTrace();
    }
  }
...
class FileSearch2 implements Runnable {

  private String initPath;
  private String fileName;

  public FileSearch2(String initPath, String fileName) {
  ...
  private void directoryProcess(File file) throws
    ...
    if (list != null) {
      for (int i = 0; i < list.length; i++) {
        ...
        if (isInterrupted) {
          System.out.println("quit from fileProcess.");
          //先判断再退出，不抛出异常
          return;
        }
